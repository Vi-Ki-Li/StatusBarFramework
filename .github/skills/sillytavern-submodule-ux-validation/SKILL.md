...
description:
  ...
  不显示”，必须使用本 Skill。
user-invocable: true
argument-hint: '[目标地址，可选，默认 http://localhost:8000] [target: 子模块名] [depth: standard/deep]'
---

# SillyTavern Submodule UX Validation

本 Skill 用于五模块可达之后的“可完成性与可理解性”验证，不是入口冒烟测试替代，也不是截图收集器。
...
3. 管理器入口已在桌面和移动端验证可达。
4. 深测前已导入测试数据（优先 `docs/.local-data/dso-terminal-imports`）。

若深测未完成数据导入，输出 `blocked-by-fixture`，不得判通过。
...
执行顺序强制：先 click，再脚本。脚本不得替代 click 结论。
...
每完成一个动作，立刻输出一条记录，不得堆到最后统一总结：

- `action`
- `screenshot`
- `result` (`pass`/`fail`/`blocked`)
- `findingType` (`interaction`/`layout`/`visual`/`state`)
- `one-line-conclusion`

当截图数量持续增加但结论区没有同步增长，视为流程违规。

## 视觉/CSS 问题最小检查集（新增）

除可用性外，每个子模块至少检查以下基础 UI 项：

1. 是否出现塌缩、重叠、遮挡。
2. 是否可正常滚动（该滚能滚，不该滚不乱滚）。
3. 是否有对齐断层、文本溢出、明显配色突兀。
4. 是否出现 `undefined/null/[object Object]` 等裸值。

发现遮挡核心按钮、无法滚动到关键区域，至少判 P1；阻塞主流程判 P0。
