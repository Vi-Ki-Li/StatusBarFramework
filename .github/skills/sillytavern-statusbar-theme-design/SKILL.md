...
- 不刻意放大文本；优先保证密度和整屏可读性。
- 建议区间：标签 11-13px、正文 13-16px、强调值 15-20px。
- 仅当用户明确提出“海报感/大字冲击”时，才允许超出上述范围。
...
- `autoEqualizeItems`：仅对 `flex-row` 容器生效，让直接子条目自动按 $1/N$ 等分宽度（1 项 100%，2 项 50%，3 项 33.3%）。
- `showLabel` / `collapsible`：容器标题与折叠开关。
- `autoSyncFromCategory` / `bindCategoryId` / `categorySortStrategy`：分类动态绑定与排序策略。

布局层与样式层边界（必须遵守）：

- 结构规则（等分、行列分布、容器折叠、绑定关系）必须放在 Layout JSON，不要固化到 Style/globalTheme。
- Style/globalTheme 只负责视觉皮肤；避免写入依赖具体 `layout node id` 的结构性宽度规则。

折叠稳定性约束：

- 折叠状态键应使用“布局作用域 + 节点 ID”联合键，避免不同布局复用节点 ID 时互相污染。
- 若出现“偶发无法展开”，优先排查状态键冲突与过重动画样式，而非先改业务数据。

模块自动保存开关约束：

- 自动保存策略应按模块独立可控，并持久化到 `SystemConfig.managerAutoSave.<moduleName>`。
- 自动保存关闭时，保留脏标记与手动保存入口，不应静默丢失用户改动。
